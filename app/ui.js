...
async function probeBackend(){
  ...
  // 2. 单独探测 TradeCheck 子系统(mrdk 部署时存在 /api/tradecheck/health)
...
async function run(){$("#err").textContent="";
  try{
    ...
    if(!dealText&&images.length){
      if(!BACKEND.ok)throw new Error("图片识别需要后端:请先运行 app/server.py,或在 TRADECHECK_BACKEND 中配置 mrdk 公网域名(或改用 CSV 交割单)。");
      setBusy(true,"正在识别图片中的交割单…");
      ...
      const j=await r.json();if(!r.ok)throw new Error(j.error||"图片识别失败");
      dealText=j.csv;
    }
