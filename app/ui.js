...
async function probeBackend(opts){
  // 云托管冷启动可能要 5-10 秒,默认 10s;首屏渲染时如果 4s 内没回就先标"检测中",后续探测覆盖
  const timeoutMs=(opts&&opts.timeoutMs)||10000;
  const el=$("#beStatus");
  try{
    const [mainRes,tcRes]=await Promise.allSettled([
      fetchTimeout(apiURL("/api/health"),timeoutMs).then(async r=>r.ok?r.json():null).catch(()=>null),
      fetchTimeout(apiURL("/api/tradecheck/health"),timeoutMs).then(async r=>r.ok?r.json():null).catch(()=>null),
    ]);
  ...
  if(BACKEND.tc){
    ...
    el.innerHTML="○ 后端唤醒中 — 点击「生成诊断报告」时会自动重试";
    el.className="be off";
  }
}

async function ensureBackendReady(){
  // 用户点上传前若 BACKEND.tc 还没就绪,先重试一次(可能是首次进页时云托管冷启动)
  if(BACKEND.tc)return true;
  await probeBackend({timeoutMs:15000});
  return BACKEND.tc;
}
...
function renderThumbs(){
  const w=$("#thumbs"),strip=$("#thumbStrip"),z=$("#imgZone");
  if(!w||!strip||!z)return;
  ...
  const ic=$("#imgCount");if(ic)ic.textContent=n?`已选 ${n} 张 · 按顺序拼接 · 点击空白处继续添加`:"";}
function syncRun(){const btn=$("#runBtn");if(btn)btn.disabled=!(fileDeal||images.length);}
...
async function run(){
  ...
  try{
    ...
    if(!dealText&&images.length){
      ...
      let useBatchOcr=BACKEND.tc&&BACKEND.tcCaps&&BACKEND.tcCaps.ocr&&BACKEND.tcCaps.parse_csv;
      let useTcOcr=useBatchOcr||(BACKEND.tc&&BACKEND.tcCaps&&BACKEND.tcCaps.extract_csv);
      if(!useTcOcr&&!BACKEND.ok){
        // 可能首屏探测因冷启动超时, 用户准备上传时再唤醒后端一次
        setBusy(true,"正在唤醒后端服务(首次访问可能需要 10-15 秒)…");
        await ensureBackendReady();
        useBatchOcr=BACKEND.tc&&BACKEND.tcCaps&&BACKEND.tcCaps.ocr&&BACKEND.tcCaps.parse_csv;
        useTcOcr=useBatchOcr||(BACKEND.tc&&BACKEND.tcCaps&&BACKEND.tcCaps.extract_csv);
      }
      if(!useTcOcr&&!BACKEND.ok){
        throw new Error("后端未响应。可能是云托管冷启动失败,稍后重试;或改用 CSV/Excel 交割单上传(不需要后端 OCR)。");
      }
...
function bindSamplePicker(ov){
  ...
  list.addEventListener("click",e=>{
    let el=e.target;
    while(el&&el!==list){
      if(el.classList&&el.classList.contains("sample-item")&&el.dataset.id){
        e.preventDefault();
        loadSampleReport(el.dataset.id);
        return;
      }
      el=el.parentElement;
    }
...
function showSamplePicker(){
  ...
  if(!ov){
    ...
    ov.innerHTML=`<div class=sample-panel role=dialog aria-label=样例报告 onclick=event.stopPropagation()>
      <div class=sample-panel-hd><h3>样例报告</h3><p>覆盖 20–90 分不同表现，每份约 200 笔完整交易。含模拟账户、真实脱敏交割单与公开打板样例。</p>
      <button type=button class=sample-close aria-label=关闭>✕</button></div>
      <div class=sample-list></div></div>`;
    document.body.appendChild(ov);
    ov.addEventListener("click",e=>{if(e.target===ov)closeSamplePicker();});
    ov.querySelector(".sample-close").addEventListener("click",closeSamplePicker);
    bindSamplePicker(ov);
  }
  const list=ov.querySelector(".sample-list");
  if(list){
...
async function loadSampleReport(id){
  if(typeof SAMPLE_DEALS==="undefined"||!SAMPLE_DEALS[id]){
    console.warn("[loadSample] missing",id);
    return;
  }
  closeSamplePicker();
  currentSampleId=id;
  fileDeal=SAMPLE_DEALS[id];
  fileMkt=(typeof SAMPLE_MKTS!=="undefined"&&SAMPLE_MKTS[id])?SAMPLE_MKTS[id]:null;
  images=[];
  renderThumbs();
  syncRun();
  showReportBusy("正在切换样例报告…");
  try{await run();}
  catch(e){console.error("[loadSample]",e);showReportBusy("切换失败，请重试");setTimeout(showReportBusy,2000);}
  finally{showReportBusy("");}
}
...
function isMobileView(){return window.innerWidth<760||screen.width<760;}
...
function unlockReportScroll(){
  const s=document.documentElement.style,b=document.body.style,r=$("#report");
  s.height="auto";s.minHeight="0";s.maxHeight="none";s.overflowY="scroll";s.overflowX="hidden";
  b.height="auto";b.minHeight="0";b.maxHeight="none";b.overflowY="visible";b.overflowX="hidden";
  b.webkitOverflowScrolling="touch";b.position="relative";
  if(r){r.style.display="block";r.style.height="auto";r.style.maxHeight="none";r.style.overflow="visible";r.style.position="relative";}
}
...
function renderReport(R){
  ...
  scheduleChartResize();
  bindChartResize();
  const sh=$("#sampleHeroBtn");if(sh)sh.onclick=showSamplePicker;
  unlockReportScroll();
...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  window.showSamplePicker=showSamplePicker;
