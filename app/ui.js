...
async function probeBackend(){
  ...
  if(BACKEND.tc){
    const caps=[];
    caps.push("自动补行情");
    if(BACKEND.tcCaps&&BACKEND.tcCaps.extract_csv)caps.push("图片 OCR 解析");
...
async function run(){$("#err").textContent="";
  try{
    ...
    if(!dealText&&images.length){
      // 公网部署优先走 mrdk 后端的 /api/tradecheck/extract_csv(RapidOCR + LLM)
      // 本地开发回退到 server.py 的 /api/extract(图像视觉模型,可选)
      const useTcOcr=BACKEND.tc&&BACKEND.tcCaps&&BACKEND.tcCaps.extract_csv;
      ...
      const ep=useTcOcr?"/api/tradecheck/extract_csv":"/api/extract";
      const r=await fetch(apiURL(ep),{method:"POST",headers:{"content-type":"application/json"},
        body:JSON.stringify({images:images.map(i=>i.url)})});
      const j=await r.json();if(!r.ok)throw new Error(j.detail||j.error||("图片识别失败 HTTP "+r.status));
      dealText=j.csv;
