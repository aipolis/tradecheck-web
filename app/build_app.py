...
body.report-mode #report{display:block!important;height:auto!important;min-height:0!important;max-height:none!important;overflow:visible!important;padding-bottom:max(140px,env(safe-area-inset-bottom))}
...
body.report-mode #report{padding-bottom:max(160px,env(safe-area-inset-bottom))}
#report .panel,#report .problem,#report .checklist,#report .scorewrap,#report .idbox{overflow:visible}
#report .checklist{grid-template-columns:1fr;gap:8px}
}
...
.up-hero{text-align:center;padding:0 0 8px}
.hero-strip{position:relative;overflow:hidden;background:linear-gradient(135deg,#1a4570 0%,#1f4e79 55%,#2e75b6 100%);color:#fff;border-radius:16px;padding:18px 24px 16px;box-shadow:0 8px 28px rgba(31,78,121,.18)}
.hero-strip::after{content:"";position:absolute;width:180px;height:180px;right:-50px;top:-70px;background:radial-gradient(circle,rgba(255,255,255,.12) 0%,transparent 70%);pointer-events:none}
.hero-strip>*{position:relative;z-index:1}
.up-hero .logo{display:inline-flex;align-items:center;gap:8px;color:rgba(255,255,255,.92);font-weight:700;font-size:14px;letter-spacing:.02em}
.up-hero .logo .dot{width:9px;height:9px;border-radius:3px;background:#8fd0ff;box-shadow:0 0 8px rgba(143,208,255,.5)}
.hero-strip h1{margin:10px 0 0;font-size:clamp(20px,4.5vw,28px);font-weight:800;line-height:1.32;letter-spacing:-.02em}
.hero-body{max-width:560px;margin:18px auto 0;padding:0 4px}
.hero-body .intro{margin:0;color:#4a5568;font-size:14.5px;line-height:1.7}
.hero-tags{display:flex;flex-wrap:wrap;gap:8px;justify-content:center;margin:14px 0 0}
.htag{font-size:12px;padding:6px 14px;border-radius:999px;background:var(--card);border:1px solid var(--line);color:var(--accent);box-shadow:0 1px 6px rgba(31,39,51,.05);font-weight:600}
.home .be{margin:14px auto 0;max-width:480px}
.upload-sec{background:var(--card);border:1px solid var(--line);border-radius:18px;padding:22px 20px 20px;margin-top:22px;box-shadow:var(--shadow)}
.sec-label{text-align:center;font-size:13px;font-weight:700;color:var(--accent);margin:0 0 16px;letter-spacing:.06em;text-transform:none}
.sec-label::before,.sec-label::after{content:"";display:inline-block;width:28px;height:1px;background:linear-gradient(90deg,transparent,var(--line));vertical-align:middle;margin:0 10px}
.sec-label::after{background:linear-gradient(90deg,var(--line),transparent)}
...
.zone .fname:empty{display:none;padding:0;margin:0}
...
.actions{text-align:center;margin-top:22px;padding:0 4px;display:flex;flex-direction:column;align-items:center;gap:12px}
...
.feat{position:relative;background:var(--card);border:1px solid var(--line);border-radius:14px;padding:20px 16px 18px;box-shadow:0 2px 10px rgba(31,39,51,.04);transition:box-shadow .2s,transform .15s;overflow:hidden}
...
.feat:hover{box-shadow:0 10px 26px rgba(31,39,51,.1);transform:translateY(-3px)}
.feat-num{display:inline-flex;align-items:center;justify-content:center;width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#eef5fc,#dceaf8);font-size:16px;margin-bottom:10px}
...
header.hero{background:linear-gradient(135deg,#1f4e79,#2e75b6);color:#fff;border-radius:16px;padding:22px 26px 20px;margin:12px 0 16px;position:relative;padding-right:max(26px,128px)}
header.hero h1{margin:6px 0 4px;font-size:24px}header.hero .meta{opacity:.92;font-size:13px;padding-right:8px}
.sample-hero-btn{position:absolute;right:16px;top:16px;background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.42);color:#fff;border-radius:20px;padding:6px 14px;font-size:12px;cursor:pointer;font-family:inherit;transition:background .15s}
...
.up-hero{padding:0 0 4px}
.hero-strip{padding:16px 18px 14px;border-radius:14px}
.hero-strip h1{font-size:20px;margin-top:8px}
.hero-body{margin-top:14px}
.hero-body .intro{font-size:13.5px;line-height:1.65}
.hero-tags{gap:6px;margin-top:12px}
.htag{font-size:11px;padding:5px 11px}
.upload-sec{padding:18px 14px 16px;margin-top:18px;border-radius:14px}
...
header.hero{padding:16px 14px 16px;border-radius:12px;margin:8px 0 14px;padding-right:14px}
header.hero h1{font-size:20px;line-height:1.3;padding-right:0;margin-top:4px}
header.hero .meta{font-size:12px;line-height:1.55;padding-right:108px}
.sample-hero-btn{right:8px;top:8px;font-size:13px;padding:9px 16px;min-height:38px;line-height:1}
...
.relay-card{border:1px solid var(--line);border-radius:10px;padding:12px 14px;margin-bottom:10px;background:#fafbfd}
.rc-head{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px}
.rc-name{font-weight:700;font-size:14px}.rc-name .code{font-weight:400;color:var(--mut);font-size:11px}
...
<div id=upload class="wrap home">
  <div class=up-hero>
    ...
    <div class=hero-body>
      <p class=intro>AI 帮你复盘交易行为：识别风格、找出问题、给出整改建议。交割单在本地解析，数据不外传。</p>
      <div class=hero-tags>
  ...
  <div class=cols>
    <div class=feat><div class=feat-num>🎯</div><h4>自动识别风格</h4><p>超短 / 打板接力 / 低吸 / 波段 / 价投，自动判定并采用对应评价标尺。</p></div>
    <div class=feat><div class=feat-num>📊</div><h4>确定性指标</h4><p>FIFO 配对重建每笔交易，胜率、盈亏比、持有周期、成本全部由代码算出。</p></div>
    <div class=feat><div class=feat-num>🧭</div><h4>规则 + AI 诊断</h4><p>规则引擎找出核心问题；接入后端时 AI 可润色小结与建议。</p></div>
  </div>
</div>
"""
